...
If you prefer an SDK-first flow for hosted MCP, see:

- `/getting-started/client-api-sdk-quickstart`

## 1. Create an API key

Recommended: use the OMNI app UI (Settings -> API). The control-plane endpoint below is what the UI uses during internal beta.
...
```bash
curl "https://api.turos.app/v1/fred/search?q=unemployment&limit=5" \
...
```ts
const url = new URL("https://api.turos.app/v1/fred/search");
...
res = requests.get(
    "https://api.turos.app/v1/fred/search",
...
func main() {
  u, _ := url.Parse("https://api.turos.app/v1/fred/search")
  q := u.Query()
...
```bash
curl "https://api.turos.app/v1/mcp/tools" \
  -H "Authorization: Bearer omni_live_..."
```
...
```bash
curl -X POST "https://api.turos.app/v1/mcp/invoke" \
  -H "Authorization: Bearer omni_live_..." \
...
```bash
# 1) initialize
curl -X POST "https://api.turos.app/mcp" \
  -H "Authorization: Bearer omni_live_..." \
  -H "Content-Type: application/json" \
  -d '{
    "jsonrpc": "2.0",
...
```bash
curl "https://api.turos.app/v1/openapi.json" \
  -H "Authorization: Bearer omni_live_..."
```
