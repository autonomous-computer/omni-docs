...
| --- | --- | --- |
| `fred.read` | Read access to FRED REST endpoints and FRED MCP tools. | `GET /v1/fred/*`, `tools/call` for `fred.*`, `POST /v1/mcp/invoke` with `fred.*` tool ids |
| `mcp.tools.read` | Read the tool catalog. | `GET /v1/mcp/tools`, hosted `tools/list` over `POST /mcp` |
| `mcp.invoke` | Invoke tools. | `POST /v1/mcp/invoke`, hosted `tools/call` over `POST /mcp` |
