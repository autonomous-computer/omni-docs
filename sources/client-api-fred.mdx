...
```bash
curl "https://api.secapi.ai/v1/fred/search?q=inflation&limit=5" \
...
```bash
curl -X POST "https://api.secapi.ai/mcp" \
  -H "x-api-key: secapi_live_..." \
...
```bash
curl "https://api.secapi.ai/v1/fred/series/CPIAUCSL?limit=24" \
  -H "x-api-key: secapi_live_..."
```
